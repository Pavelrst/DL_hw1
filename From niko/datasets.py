...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        picture=torch.randint(0,255,size=self.image_dim,dtype=torch.uint8)
        picture_class=np.random.randint(0,self.num_classes-1)
        return picture , picture_class
        # ========================
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # ====== YOUR CODE: ======
        if self.offset + index >= len(self.source_dataset) or index >= self.subset_len  :
            raise IndexError()
